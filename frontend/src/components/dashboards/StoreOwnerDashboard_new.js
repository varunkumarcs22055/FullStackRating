    ...
    const fetchStoreData = async () => {
        try {
            ...
            if (data.success) {
                ...
                const localStores = JSON.parse(localStorage.getItem('myStores') || '[]');
                if (localStores.length > 0) {
                    setStore(localStores[0]);
                    setAverageRating(localStores[0].average_rating || 0);
                }
            }
        } catch (error) {
    ...
    const handleAddStore = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                localStorage.setItem('myStores', JSON.stringify(localStores));
                
                setSuccess('Store added successfully!');
                setNewStoreData({ name: '', email: '', address: '' });
                fetchStoreData();
                setTimeout(() => {
                        ...
                        {store ? (
                            <div style={{ padding: '20px', border: '1px solid #ddd', borderRadius: '8px', backgroundColor: '#f9f9f9' }}>
                                ...
                                <div style={{ marginTop: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    ...
                                    <span style={{ color: '#666', fontSize: '14px' }}>({store.total_ratings || ratings.length} reviews)</span>
                                </div>
                            </div>
                        ) : (
                            <div style={{ textAlign: 'center', padding: '40px', color: '#666' }}>
                                ...
                                <button 
                                    style={{ padding: '10px 20px', backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', marginTop: '10px' }}
                                    onClick={() => setShowAddStoreModal(true)}
                                >
                    ...
                    <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '8px', width: '90%', maxWidth: '500px', maxHeight: '90vh', overflow: 'auto' }} onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleUpdatePassword}>
                            ...
                            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '20px' }}>
                                ...
                                <button type="button" style={{ padding: '10px 20px', backgroundColor: '#6c757d', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }} onClick={() => setShowPasswordModal(false)}>
                                    Cancel
                                </button>
                            </div>
                        </form>
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }} onClick={() => setShowAddStoreModal(false)}>
                    <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '8px', width: '90%', maxWidth: '500px', maxHeight: '90vh', overflow: 'auto' }} onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleAddStore}>
                            ...
                            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '20px' }}>
                                <button type="submit" style={{ padding: '10px 20px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>Add Store</button>
                                <button type="button" style={{ padding: '10px 20px', backgroundColor: '#6c757d', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }} onClick={() => setShowAddStoreModal(false)}>
                                    Cancel
                                </button>
                            </div>
                        </form>
